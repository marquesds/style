...
This document is the one-page version of the philosophy that the rules, skills, and commands in this repository encode. It is written in normal English so newcomers can read it quickly. The rule and skill files themselves are written in caveman style for token efficiency once an agent has loaded them.

## What this is

A single source of truth for one engineer's coding style, distributed as native files for any AI coding agent: Claude Code, Cursor, Codex, Vibe. You clone the repo, run `./install.sh`, and the same practices appear in every agent you use.
...
**Test-driven development.** Write a failing test for the next behavior, then write the smallest implementation that makes it pass, then refactor while the test keeps you honest. Tests are how we prove the code does what it claims; they come first, not last.
...
**Minimal hexagonal architecture.** The domain knows nothing about its surroundings; adapters know about the domain. We name the layers however the language and project want, but the dependency direction always points inward toward pure business rules.
...
**Source-driven development.** Framework code is verified against the official docs of the version installed, not against memory. Every nontrivial framework decision cites a URL.
...
- `source/rules/` — always-on guidance. Six files: agent workflow, subagent-first, code quality, design principles, documentation, observability.
- `source/skills/` — on-demand expertise. Sixteen files covering everything described above.
