...
1. **Documentation-only.** Write the contract in docstrings (`# Preconditions`, `# Postconditions`, `# Invariants`). Nothing enforces it beyond convention and review. Cheap; entirely unenforced.
...
Option 3, with explicit fallbacks. The `liskov-and-design-by-contract` skill tells you to:
...
**Negative.** Languages with weaker type systems (loose dynamic languages, older Python, plain JS) get fewer guarantees from this approach. You still have to learn the types-first idioms in each language; the skill calls them out, but practice takes time.

**Mitigations.** The skill's GOOD/BAD examples show the most common types-first patterns. Runtime asserts and property tests are explicitly listed as the fallback when types fall short — there's no purity test that forces you to skip them.
