...
description: >
  Tests + lint + types + coverage + observability + perf budget + 5-axis review.
...
Substitute idiomatic equivalents per language (`cargo test && cargo clippy -- -D warnings`, `mix test && mix dialyzer`, `pnpm test && pnpm tsc --noEmit`).
